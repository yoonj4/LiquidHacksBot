...
module.exports = async function insertCharacter(character) {
    ...
    connection.connect();
    let [rows, fields] = await connection.execute('insert into smash_game.character(discord_tag, prize_money, experience, name, stamina, is_pro) values(?, ?, ?, ?, ?, ?)', [character.discord_tag, character.prize_money, character.experience, character.name, character.stamina, character.is_pro]);
