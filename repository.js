...
module.exports = async function insertCharacter(character) {
    ...
    connection.connect();
    let [rows, fields] = await connection.execute('insert into smash_game.character(prize_money, experience, name, stamina, is_pro, is_resting, discord_id) values(?, ?, ?, ?, ?, ?, ?)', [character.prize_money, character.experience, character.name, character.stamina, character.is_pro, character.is_resting, character.discord_id]);
